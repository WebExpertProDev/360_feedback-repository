class Users::InvitationsController < Devise::InvitationsController
  ...
  def create
    if current_user.email == invite_params[:email]
      return redirect_back(fallback_location: user_tests_path,alert: "You cannot invite yourself. Please enter another email address.")
    end
  ...
  protected
    ...
    def invite_resource(&block)
      ...
      if @user.persisted?
        BasicMailer.send_invite(invite_params[:email],invite_params[:test_id]).deliver_now
      else
      	@user.password = ('a'..'z').to_a.shuffle[0,8].join
      	@user.save!
        @user.invite!(current_user)
      end
      TestInvite.find_or_create_by!(inviter_id: current_user.id,invitee_id: @user.id,leadership_survey_id: invite_params[:test_id]) 
      @user
    	# # @user.invite_to(invite_params[:test_id],current_user)
